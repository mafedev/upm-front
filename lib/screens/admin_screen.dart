    ...
    final result = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(child: const Text("Reiniciar"), onPressed: () => Navigator.pop(context, true)),
        ],
      ),
    );
...
widget.serialService.send('1234');
      if (mounted) {
        ScaffoldMessenger.of(context)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(child: !authenticated ? _buildLoginCard() : _buildAdminPanel()),
    );
  }
          ...
          child: Padding(
            ...
            child: GridView(
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount: 2, crossAxisSpacing: 15, mainAxisSpacing: 15, childAspectRatio: 2),
              children: [
                _dashboardButton(
                  icon: Icons.add_task,
                  label: "Cargar sesiones",
                  color: const Color(0xFF43A047),
                  onTap: () => _openInput(1, "Número de sesiones"),
                ),
                _dashboardButton(
                  icon: Icons.edit,
                  label: "Cambiar número de serie",
                  color: const Color(0xFF1E88E5),
                  onTap: () => _openInput(4, "Nuevo número de serie"),
                ),
                _dashboardButton(
  ...
  Widget _dashboardButton({required IconData icon, required String label, required Color color, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(label, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
            ),
          ],
        ),
      ),
  ...
  void _openInput(int command, String label) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => InputScreen(serialService: widget.serialService, command: command, label: label),
      ),
    );
  }
}